...
export const removeRocket = (rocket) => {
  return dispatch => {
      ...
      const userKey = localStorage.getItem('token')
      axios
        .post("http://127.0.0.1:8000/removerocket/", rocket, { 'headers': { 'Authorization': `token ${userKey}` }})
        .then(response => {
          return axios.post("http://127.0.0.1:8000/getrocketsbyclassname/", {className: rocket.className}, { 'headers': { 'Authorization': `token ${userKey}` }})
            .then(response => {
              dispatch({ type: GET_ROCKETS_BY_CLASS, payload: response.data });
            })
        })
